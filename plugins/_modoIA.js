...
handler.before = async (m) => {
  ...
  if (bot.modoia && !m.isGroup && !Prefijo && !m.fromMe && m.text !== '') {
     if (/^.*false|disnable|(turn)?off|0/i.test(m.text)) return;
        ...
        const sytm = await fetch(`https://https://raw.githubusercontent.com/Edwardofc/ATRO-BOT-MD/master/atro.sh/JSON/chatgpt_indicciones.txt`).then(v => v.text());
        const sistema1 = sytm.replace('@name', namedem)
        const sistema2 = 'Tu seras Atro - Bot - MD, un bot de WhatsApp creado por Edwardofc';
        try {
